...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  importArticles({ rankUrls: rankUrls.length ? rankUrls : [DEFAULT_RANK_URL] })
    .then(result => console.log(`导入完成：成功 ${result.imported.length} 篇，失败 ${result.failed.length} 篇，共 ${result.bytes} 字节。`))
