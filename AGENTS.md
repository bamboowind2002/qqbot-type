...
- 管理员 QQ `1144107042` 可在私聊中使用 `码表管理` 命名空间维护公共方案和任意用户的私人方案，包括列表、查看、按 QQ 精确查找私人方案、上传、取消上传、设置配置和删除；按 QQ 查询命令为 `码表管理 查QQ <QQ号>`。管理员命令只解析当前消息的文本段，不能把被引用文件的内容拼入命令。管理员上传必须在命令末尾显式添加 `替换` 才能覆盖既有公共方案或目标 QQ 的私人方案；删除使用五分钟有效的一次性确认码。管理员操作复用全局单并发构建队列、用户锁、方案名锁和原子版本发布，不能绕过这些协调器直接修改数据库或方案文件。
