...
- 私聊中回复一个离线 `.txt` 文件并发送 `上传词提 <方案名>` 会校验文件类型、30 MiB 上限和方案名。下载可并行；`dist/hintBuildManager.js` 用全局单并发队列启动一次性 `dist/hintBuildWorker.cjs` 子进程，由子进程同步调用 `save_table()`，因此构建不会阻塞机器人主事件循环，也不会让两个大码表的构建内存和临时磁盘峰值叠加。
