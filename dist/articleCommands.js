...
export const ARTICLE_HELP = `发文帮助
-文 [关键词] [页码]：查看文章列表
...
-进 [+/−/=数字]：查看已发送字数，或手动调整下一段起点

二、三种普通发文
-顺 [字数] [标题]：从数据库进度发送下一段；填写标题时同时选中该文章
-随 [字数] [标题]：随机截取连续段落
-乱 [字数] [标题] [起点-终点]：从有效行范围随机拼接，行下标不重复，最后一行可截断
...
四、会话与自动续段
-上：顺序发文回到上一段；会话失效或重启后仍可使用
-下：从当前进度发送一段；没有会话时按上次配置恢复，只发送一条消息
-发：重复最近一段且不改变进度；没有分段记录时才从当前进度发送
